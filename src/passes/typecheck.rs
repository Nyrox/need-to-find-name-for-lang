...
fn check_top_level_declaration(
    context: &mut Context,
    declaration: &mut untyped::TopLevelDeclaration,
) {
    match declaration {
        untyped::TopLevelDeclaration::FunctionDeclaration(ident, block, rType, params) => {
            ...
            if let Some(expr) = &block.return_expr {
                block
                    .statements
                    .push(untyped::Statement::ReturnStatement(*expr.clone()));
            }

            let statements: Vec<_> = block
                .statements
                .iter()
                .filter_map(|s| check_statement(context, s))
                .collect();

            println!("VarTable {}: {:#?}", ident, context.current_scope);

            context.typed_ast.functions.insert(
                ident.clone(),
                typed::FunctionDefintion {
...
fn check_statement(
    ...
    match statement {
        ...
        untyped::Statement::VariableAssignment(s, e) => {
            return Some(typed::Statement::VariableAssignment(
                context.current_scope.find(s).unwrap().varstack_offset,
                check_expression(context, e),
            ))
        }
        untyped::Statement::ReturnStatement(e) => {
            return Some(typed::Statement::ReturnStatement(check_expression(
                context, e,
            )));
        }
        untyped::Statement::ExpressionStatement(e) => {
            return Some(typed::Statement::ExpressionStatement(check_expression(
                context, e,
            )));
        }
        untyped::Statement::PrintStatement(e) => {
            return Some(typed::Statement::PrintStatement(check_expression(
                context, e,
            )));
        }
...
fn check_block(context: &mut Context, block: &untyped::Block) -> typed::Block {
    typed::Block {
        ...
        return_expr: block
            .return_expr
            .clone()
            .map(|e| Box::new(check_expression(context, &e.clone()))),
    }
}
...
fn check_expression(context: &mut Context, expression: &untyped::Expression) -> typed::Expression {
    match expression {
        untyped::Expression::BinaryOperation(l, o, r) => {
            ...
            return typed::Expression::BinaryOperation(Box::new(left), *o, Box::new(right), _type);
        }
        ...
        untyped::Expression::FunctionCall(ident, params) => {
            match context.typed_ast.functions.get(ident) {
                Some(f) => typed::Expression::FunctionCall(
                    ident.clone(),
                    f.return_type,
                    params
                        .iter()
                        .map(|e| check_expression(context, e))
                        .collect(),
                ),
                None => panic!(
                    "{} is not a function\n\n{:?}",
        ...
        untyped::Expression::Block(block) => {
            return typed::Expression::Block(typed::Block {
                ...
                return_expr: block
                    .return_expr
                    .clone()
                    .map(|e| Box::new(check_expression(context, &*e))),
            });
        }
