...
use repr::{self, instruction_set, linked};
...
impl Machine {
    ...
    pub fn execute(&mut self) {
        ...
        loop {
            ...
            isp += 1;
            if isp >= self.module.instructions.len() {
                break;
            }
        }
    }
    ...
    pub fn push_var(&mut self, var_i: i16) {
        unsafe {
            ::std::ptr::copy::<i64>(
                transmute(&self.var_table[var_i as usize]),
                transmute(&self.stack[self.stack_top]),
                1,
            );
            self.stack_top += 1;
        }
    ...
    pub fn get_constant<T>(&self, i: i16) -> T {
        unsafe {
            let v = ::std::mem::zeroed::<T>();
            ::std::ptr::copy::<T>(
                transmute(&self.module.constants[i as usize]),
                transmute(&v),
                1,
            );
            return v;
